...
schedule.scheduleJob(rule1, () =>
  logger.info("매분" + rule1.second + "초 마다 수행!")
);
...
schedule.scheduleJob(rule2, () =>
  logger.debug("매시간" + rule2.minute + "분" + rule2.second + "초 마다 수행!")
);
...
schedule.scheduleJob(rule3, () =>
  logger.warn("매시간" + rule3.minute + "분" + rule3.second + "초 마다 수행!")
);
...
schedule.scheduleJob(rule4, () =>
  logger.debug("매주 월~금 매 분 45초마다 실행")
);
